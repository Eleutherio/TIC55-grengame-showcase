...
export default function MissaoVideo({
  mission,
  onComplete,
  isCompleted,
  completion,
  ...
  const minWatchSeconds = (mission.content_data.duration || 1) * 60;
  ...
  useEffect(() => {
    if (isCompleted) {
      setVideoWatched(true);
      setServerProgressSeconds((current) => Math.max(current, minWatchSeconds));
    }
  }, [isCompleted, minWatchSeconds]);
  ...
  }, [isCompleted, isPlaying, mission.id, sendConsumptionHeartbeat]);
  ...
  }, [videoId, isCompleted, sendConsumptionHeartbeat]);
    ...
    if (isCompleted) {
      try {
        ...
        const trailResponse = await fetch(
          `${API_URL}/auth/games/${mission.game}/trilha/`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
        ...
        console.error("Erro ao buscar proxima missao:", error);
        navigate(`/app/trilhas/${mission.game}`);
      }
      return;
    }
    ...
    try {
      ...
      const message =
        error instanceof Error
          ? error.message
          : "Nao foi possivel validar a conclusao do video.";
    ...
    >
      {isCompleted && (
        ...
        >
          ...
          <div>
            ...
            >
              Voce pode assistir novamente, mas nao ganhara pontos extras.
            </p>
          </div>
        </div>
      )}
      ...
      >
        ...
        >
          <div
            style={{
              width: `${
                Math.min(serverProgressSeconds / Math.max(minWatchSeconds, 1), 1) *
                100
              }%`,
              height: "100%",
        ...
        >
          {mission.order} de {totalMissions} Conteudos
        </p>
      </div>
      ...
      {loadError && (
        ...
        >
          <p style={{ margin: "0 0 8px", fontWeight: "600", color: "#856404" }}>
            ⚠️ Nao foi possivel carregar o video
          </p>
          <p style={{ margin: 0, fontSize: "14px", color: "#856404" }}>
            Recarregue a pagina ou desative o bloqueador. A conclusao so e
            liberada quando o consumo do video e sincronizado com o servidor.
          </p>
        </div>
      )}
      ...
      >
        ...
        >
          {videoId ? (
            ...
            >
              <p>URL de video nao disponivel</p>
            </div>
          )}
        </div>
      </div>
      ...
      >
        ...
        <button
          ...
          style={{
            ...
            fontWeight: "600",
            backgroundColor:
              (videoWatched || isCompleted) && !isCompleting
                ? "#FFC107"
                : "#cccccc",
            color: "#1E2875",
            border: "none",
            ...
            gap: "8px",
            opacity:
              (videoWatched || isCompleted) && !isCompleting ? 1 : 0.6,
          }}
          onMouseOver={(e) => {
      ...
      {!videoWatched && !isCompleted && (
        ...
        >
          ...
          >
            ⏱️ Tempo validado: {formatTime(serverProgressSeconds)} /{" "}
            {formatTime(minWatchSeconds)}
          </div>
          <p style={{ margin: 0 }}>Assista ao video ate o final para continuar</p>
        </div>
      )}
    </div>
  );
