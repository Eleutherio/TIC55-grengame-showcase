...
export default function MissaoQuiz({
  mission,
  isCompleted,
  totalMissions,
}: MissaoQuizProps) {
  ...
  useEffect(() => {
    if (isCompleted) {
      ...
      setQuizSummary(
        "Quiz já concluído. Você pode revisar as perguntas, mas não ganhará pontos extras.",
      );
    }
  }, [isCompleted]);
        ...
        if (!isCompleted) {
          ...
          fetch(`${API_URL}/auth/missoes/${mission.id}/validar/`, {
            ...
            .then(async (response) => {
              ...
              if (!response.ok) {
                return Promise.reject(
                  new Error(payload?.error || "Não foi possível validar o quiz."),
                );
              }
              return payload;
            })
            .then((data) => {
              ...
              setQuizSummary(
                data.message ||
                  `Você acertou ${data.correct_answers || 0} de ${data.total_questions || questions.length} questões.`,
              );
            ...
            .catch((error) => {
              ...
              setQuizSummary(
                error instanceof Error
                  ? error.message
                  : "Não foi possível validar o quiz.",
              );
            })
            .finally(() => {
        ...
        >
          <div
            style={{
              width: hasAnswered
                ? "100%"
                : `${((currentQuestionIndex + 1) / questions.length) * 75}%`,
              height: "100%",
          ...
          {currentQuestion.options.map((option, index) => {
            ...
            return (
              <button
                ...
                disabled={showResult}
                style={{
                  backgroundColor: isSelected ? "#E8E4FF" : "white",
                  color: "#1E2875",
                  border: isSelected ? "3px solid #5B4CCC" : "none",
                  borderRadius: "12px",
                  padding: "24px",
                  fontSize: "16px",
          ...
          >
            ...
            >
              {currentQuestionIndex === questions.length - 1
                ? "Resposta registrada. Validando quiz..."
                : "Resposta registrada. Preparando próxima questão..."}
            </p>
          </div>
        )}
          ...
          {quizFinished ? (
            ...
            <button
              ...
              style={{
                ...
                fontWeight: "600",
                backgroundColor:
                  selectedOption !== null && !showResult && !isSubmitting
                    ? "#FFC107"
                    : "#cccccc",
                color: "#1E2875",
                border: "none",
